...
const caCertificate = process.env.REMOTELINK_TEST_VNC_CA_PEM || "";
if (!baseURL || !token || !host)
  throw new Error("integration test environment is incomplete");
...
try {
  ...
  targetId = await page.evaluate(
    async ({ headers, host, port, username, password, caCertificate }) => {
      ...
      if (!response.ok)
        throw new Error(
          `create target failed: ${response.status} ${await response.text()}`,
        );
      return (await response.json()).id;
    },
    { headers, host, port, username, password, caCertificate },
  );
  ...
  await page.goto(
    `${baseURL}/vnc/session?target=${encodeURIComponent(targetId)}&name=Integration%20VNC`,
  );
  try {
    await page.locator("#status").waitFor({ state: "hidden", timeout: 12000 });
  } catch (error) {
    const diagnostics = await page
      .locator("#status-title, #status-detail")
      .allTextContents();
    throw new Error(
      `noVNC did not connect: ${diagnostics.join(" | ")} (${error.message})`,
    );
  }
  ...
  if (targetId) {
    await page
      .evaluate(
        async ({ headers, targetId }) => {
          await fetch("/api/admin/vnc/delete", {
            method: "POST",
            headers,
