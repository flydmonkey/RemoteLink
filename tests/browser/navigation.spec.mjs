...
test('RDP page exposes the protocol navigation', async ({ page }) => {
  ...
  await expect(page.locator('.protocol-link[aria-current="page"]')).toHaveText('RDP');
  ...
  await expect(page.locator('.account-admin')).toHaveText(/用户管理|User management/);
  await expect(page.locator('.account-admin')).toHaveAttribute('href', '/users');
  await expect(page.locator('#account-avatar svg')).toHaveCount(1);
  await expect(page.locator('#account-name')).toBeHidden();
  await expect(page.locator('#account-avatar')).toHaveCSS('color', 'rgb(179, 179, 179)');
  await expect(page.locator('#account-avatar')).toHaveCSS('background-color', 'rgb(54, 54, 54)');
...
test('VNC page follows the existing RemoteLink shell', async ({ page }) => {
  await page.addInitScript(() => sessionStorage.setItem('remote-gateway-access-token', 'test'));
  await page.route('**/api/auth/me', route => route.fulfill({ json: { name: 'Test User', username: 'test', admin: false } }));
  await page.route('**/api/vnc/targets', route => route.fulfill({ json: { targets: [
  ...
  await expect(page.locator('.protocol-nav a[aria-current="page"]')).toHaveText('VNC');
  await expect(page.getByRole('heading', { name: 'VNC 连接', exact: true })).toBeVisible();
  await expect(page.locator('.hero-icon svg[data-protocol-icon="vnc"]')).toHaveCount(1);
  await expect(page.locator('.hero p')).toHaveText(/选择计算机并连接|Select a computer and connect/);
  ...
  await expect(page.getByRole('button', { name: 'VNC 管理', exact: true })).toBeHidden();
  await expect(page.locator('#users-link')).toBeHidden();
  await expect(page.locator('.app')).toHaveCSS('height', '270px');
});

test('VNC administrators add connections from management only', async ({ page }) => {
  await page.addInitScript(() => sessionStorage.setItem('remote-gateway-access-token', 'test'));
  await page.route('**/api/auth/me', route => route.fulfill({ json: { name: 'Administrator', username: 'admin', admin: true } }));
  await page.route('**/api/vnc/targets', route => route.fulfill({ json: { targets: [] } }));
  await page.route('**/api/admin/vnc', route => route.fulfill({ json: { connections: [] } }));
  await page.route('**/api/admin/vnc/activity', route => route.fulfill({ json: { active: [
    { id: 's1', targetName: 'Live Desktop', username: 'test', startedAt: 1700000000, endedAt: 0 }
  ], history: [{ id: 's0', targetName: 'Old Desktop', username: 'test', startedAt: 1699990000, endedAt: 1699991000 }] } }));
  await page.route('**/api/admin/vnc/test', route => route.fulfill({ json: { reachable: true } }));
  await page.route('**/api/admin/vnc/save', route => route.fulfill({ json: { id: 'vnc-test' } }));
  await page.goto('/vnc.html');
  await expect(page.getByRole('button', { name: '添加 VNC 连接' })).toHaveCount(0);
  await expect(page.locator('#connect-form .primary')).toHaveCSS('min-width', '124px');
  await expect(page.locator('.account-menu')).not.toContainText('设置');
  await expect(page.locator('.account-menu')).not.toContainText('管理控制台');
  await expect(page.locator('#users-link')).toHaveText(/用户管理|User management/);
  ...
  await expect(page.locator('#account-trigger .avatar-icon')).toHaveCSS('color', 'rgb(179, 179, 179)');
  await expect(page.locator('#account-trigger .avatar-icon')).toHaveCSS('background-color', 'rgb(54, 54, 54)');
  await expect(page.locator('#account-name')).toHaveText('Administrator · admin');
  await page.getByRole('button', { name: 'VNC 管理', exact: true }).click();
  await expect(page.locator('#admin-nav')).toBeVisible();
  await expect(page.getByRole('heading', { name: 'VNC 连接管理' })).toBeVisible();
  await expect(page.getByRole('link', { name: '连接管理' })).toHaveAttribute('href', '#connections');
  await page.getByRole('button', { name: '添加连接', exact: true }).click();
  await expect(page.locator('#editor')).toBeVisible();
  await expect(page.locator('#editor').getByLabel('主机')).toBeVisible();
  await expect(page.locator('#editor').getByLabel(/用户名|Username/)).toBeVisible();
  ...
  await page.locator('#editor').getByRole('button', { name: '测试连接' }).click();
...
test('SSH route has a reserved navigation destination', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'SSH 功能准备中' })).toBeVisible();
});

test('VNC session uses noVNC Core with RemoteLink controls', async ({ page }) => {
  await page.addInitScript(() => sessionStorage.setItem('remote-gateway-access-token', 'test'));
  await page.route('**/api/vnc/sessions', route => route.fulfill({ json: {
    websocketUrl: '/vnc/ws?ticket=test', password: '', viewOnly: false
  } }));
  await page.route('**/vendor/novnc/core/rfb.js', route => route.fulfill({
  ...
  await expect(page.getByRole('button', { name: 'Ctrl+Alt+Del' })).toBeVisible();
  ...
  await expect(page.locator('#remote-toolbar')).toHaveCSS('flex-direction', 'column');
  ...
  await page.evaluate(() => window.testRfb.dispatchEvent(new CustomEvent('disconnect', { detail: { clean: false } })));
  await expect(page.getByRole('button', { name: '重新连接' })).toBeVisible();
  await expect(page.getByRole('link', { name: '返回连接列表' })).toHaveAttribute('href', '/vnc');
  await page.locator('#disconnect').evaluate(button => button.click());
  await expect(page).toHaveURL(/\/vnc$/);
});

test('VNC disconnect returns even when the target is unavailable', async ({ page }) => {
  await page.addInitScript(() => sessionStorage.setItem('remote-gateway-access-token', 'test'));
  await page.route('**/api/vnc/sessions', route => route.fulfill({ status: 502, json: { error: 'unavailable' } }));
...
test('VNC administration page exposes connection and permission management', async ({ page }) => {
  await page.goto('/vnc-admin.html');
  await expect(page.getByRole('heading', { name: 'VNC 连接' })).toBeVisible();
  await expect(page.getByRole('heading', { name: '用户授权' })).toBeHidden();
...
test('user management exposes the complete account lifecycle', async ({ page }) => {
  await page.addInitScript(() => sessionStorage.setItem('remote-gateway-access-token', 'test'));
  await page.route('**/api/admin/users', route => route.fulfill({ json: { items: [
    { id: 0, name: 'Administrator', username: 'admin', admin: true, enabled: true, allowedTargets: [] },
    { id: 1, name: 'Test User', username: 'test', admin: false, enabled: true, allowedTargets: [] }
  ] } }));
  await page.route('**/api/admin/state', route => route.fulfill({ json: { targets: [
    { id: 'rdp-1', name: 'Windows', managed: true }
  ] } }));
  await page.route('**/api/admin/vnc', route => route.fulfill({ json: { connections: [
    { id: 'vnc-1', name: 'Desktop' }
  ] } }));
  ...
  await expect(page.locator('#permissions').getByRole('heading', { name: 'RDP' })).toBeVisible();
  await expect(page.locator('#permissions').getByRole('heading', { name: 'VNC' })).toBeVisible();
  await expect(page.locator('#permissions').getByRole('heading', { name: 'SSH' })).toBeVisible();
  ...
  await expect(page.locator('#permissions').getByText('暂无 SSH 连接')).toBeVisible();
});
