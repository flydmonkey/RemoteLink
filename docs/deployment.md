...
The browser authenticates to the gateway with `RG_ACCESS_TOKEN`. The token is
...
`RG_ALLOWED_HOSTS` defaults to `*`, which permits any non-empty DNS name,
...
file named by `RG_TARGETS_FILE` must match that policy or the process refuses to
...
plain environment files. `config/remote-gateway.service.example` loads an
...
accepts `RG_ACCESS_TOKEN_FILE` for the mounted gateway-token credential. Never
...
uses the PEM certificate and private key configured with `RG_TLS_CERTIFICATE`
and `RG_TLS_PRIVATE_KEY`; its `/ws` route carries WebSocket signaling.
...
Keep the primary token in `RG_ACCESS_TOKEN_FILE`. To authorize additional
users with independent tokens, set `RG_ACCESS_TOKENS` to a comma-separated
